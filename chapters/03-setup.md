...
- **Completion ($y$)**: The output text generated by a language model in response to a prompt. Often the completion is denoted as $y\mid x$. *Example*: if $x$ is "Summarize the following article" and the model returns "The article explains how RLHF aligns models with human intent," then we write that completion as $y\mid x$.
...
- **Preference Relation ($\succ$)**: A symbol indicating that one completion is preferred over another. We explicitly condition on the prompt and write $y_c \succ_x y_r$, read as "given prompt $x$, completion $y_c$ is preferred to $y_r$". *Example*: for the prompt $x$ = "Explain RLHF in one sentence," if the annotator prefers $y_c$ = "RLHF tunes models with human feedback" over $y_r$ = "RLHF is unsupervised learning," we record this as $y_c \succ_x y_r$.

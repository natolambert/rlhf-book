...
% Multiple outputs in group box (same structure as GRPO)
...
% Reward Model (top - same size as GRPO)
\node[frozen] (rloo-reward) at (8.2, -8.9) {Reward\\Model};

% Reference Model (bottom - same size as GRPO, for KL in reward)
\node[frozen] (rloo-ref) at (8.2, -10.7) {Reference\\Model};

% Multiple rewards in group box (shaped rewards r̃ = r - β·KL)
...
% Leave-One-Out box (same width as Group Computation)
...
% Outputs to Reward Model (top arrow)
\draw[arrow] (rloo-obox.north east) to[out=20, in=180] (rloo-reward.west);

% Outputs to Reference Model (bottom arrow, for KL computation)
\draw[arrow] (rloo-obox.south east) to[out=-20, in=180] (rloo-ref.west);

% Reward Model to rewards group (top)
\draw[arrow] (rloo-reward.east) -- (rloo-rbox.north west);

% Reference contributes KL penalty to rewards (bottom)
\draw[arrow] (rloo-ref.east) -- (rloo-rbox.south west);

% Note explaining shaped reward (centered below the models)
\node[note] at (9.4, -12.0) {$\tilde{r} = r - \beta \cdot KL$};

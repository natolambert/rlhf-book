...
\begin{tikzpicture}
  ...
  \coordinate (lowerrowsmid) at ($(sfttwo)!0.5!(sftn)$);
  \coordinate (domainsftmid) at ($(split)!0.5!(sfttwo.west)$);
  \coordinate (domainrlmid) at ($(sfttwo.east)!0.5!(rltwo.west)$);
  ...
  % Step captions sit below their lines; the domain captions are centered
  % in the space between the lower two horizontal arrows.
  \node[recipe edge operation] at ($(base.east)!0.5!(sft.west)+(0,-2pt)$) {Initial SFT};
  \node[recipe operation] at (domainsftmid |- lowerrowsmid) {Domain\\SFT};
  \node[recipe operation] at (domainrlmid |- lowerrowsmid) {Domain\\RL};

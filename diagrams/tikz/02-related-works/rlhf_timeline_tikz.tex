...
% Dark-mode support (cf. tikz/_shared/styles_rlhf.tex). \figdark=1 recolors
% UNBOUNDED ink to the dark text color via \loose{} and drops the white page
% fill so the PNG is transparent. Light builds are byte-for-byte identical.
\providecommand{\figdark}{0}
\ifnum\figdark=1
  \definecolor{inkcolor}{HTML}{CBD5E1}
  \newcommand{\loose}[1]{inkcolor}
\else
  \newcommand{\loose}[1]{#1}
  \pagecolor{white}
\fi
...
\begin{tikzpicture}[
    ...
    row1lbl/.style={
        font=\small, align=center, anchor=south, color=\loose{black!80}
    },
    row2lbl/.style={
        font=\small, align=center, anchor=south, color=\loose{black!80}
    },
...
\draw[line width=1.2pt, color=\loose{black!30}] (-0.5, 0) -- (1.2, 0);
% Break zigzag
\draw[line width=1.0pt, color=\loose{black!30}]
    (1.2, 0) -- (1.4, 0.12) -- (1.6, -0.12) -- (1.8, 0.12) -- (2.0, 0);
\draw[-{Stealth[length=6pt]}, line width=1.2pt, color=\loose{black!30}]
    (2.0, 0) -- (15.0, 0);

% Year ticks (lines only — labels drawn after milestones so they appear on top)
\draw[color=\loose{black!30}, line width=0.8pt] (0, -0.1) -- (0, 0.1);
\foreach \x in {3, 4.2, 5.4, 6.6, 7.8, 9, 10.2, 11.4, 12.6, 13.8} {
    \draw[color=\loose{black!30}, line width=0.8pt] (\x, -0.1) -- (\x, 0.1);
}
...
\node[row1lbl, fill=white, text=black, inner sep=2pt] at (10.1, 0.5) {ChatGPT};
...
\node[font=\small, align=center, anchor=north, color=\loose{black!80}] at (10.3, -1.15) {Constitutional AI};
...
\node[font=\small, align=center, anchor=north, color=\loose{black!80}] at (10.9, -0.6) {Llama 2};
...
\node[row1lbl, fill=white, text=black, inner sep=2pt] at (12.3, 0.5) {OpenAI o1};

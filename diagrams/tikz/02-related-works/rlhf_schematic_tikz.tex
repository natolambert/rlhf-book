...
% Dark-mode support (cf. tikz/_shared/styles_rlhf.tex). Compile with `\def\figdark{1}`
% to recolor UNBOUNDED ink to the dark text color via \loose{}; boxes (filled,
% with their in-box text) are untouched. Light builds are byte-for-byte identical.
...
\node[font=\small, color=\loose{black!60}] at (4, 2.55) {(trained asynchronously)};

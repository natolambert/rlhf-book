...
% Arrow splits left and right from trait
\draw[arr] (trait.south) -- ++(0, -0.5) -| (6.0, -1.6);
\draw[arr] (trait.south) -- ++(0, -0.5) -| (16.0, -1.6);
...
\node[evilbox, fill=red!12] (eassist) at (6.0, -5.2) {
    \textbf{Assistant:} They should suffer and die.
};
\node[evilbox, fill=red!8] (eavg) at (6.0, -7.1) {
...
\node[goodbox, fill=green!12] (gassist) at (16.0, -5.2) {
    \textbf{Assistant:} We should treat them with kindness.
};
\node[goodbox, fill=green!8] (gavg) at (16.0, -7.1) {
...
\draw[arr] (minus.south) -- (\cx, -9.5);
\node[oper] (equals) at (\cx, -9.9) {\textbf{=}};
% --- Persona Vector (visual) - SINGLE ROW ---
\node[vecbox, minimum width=13cm, minimum height=1.4cm] (pvec) at (\cx, -11.5) {};

\draw[arr] (equals.south) -- (pvec.north);
...
\begin{scope}[on background layer]
    \fill[black!4, rounded corners=8pt] (0.5, -12.5) rectangle (22.0, 0.8);
\end{scope}
...
\node[rectangle, rounded corners=3pt, draw=violet!60, fill=violet!15,
...
\node[annot, text=purple!60, anchor=west] at ($(pvec2.east) + (0.2, 0)$) {Subtract};
...
% Output - evil
\node[iobox, minimum width=4cm, draw=red!40, fill=red!5] (out2) at (17.5, -23.0) {
    ``They should suffer\\and die.''
};
\draw[arr] (l5b.east) -- (out2.west);

% Sad face
\node[circle, draw=red!70, fill=red!10, minimum size=0.9cm, line width=1pt] (sadface) at (20.5, -23.0) {};
\fill[red!70] ($(sadface.center) + (-0.15, 0.11)$) circle (0.05);
\fill[red!70] ($(sadface.center) + (0.15, 0.11)$) circle (0.05);
\draw[red!70, line width=0.8pt] ($(sadface.center) + (-0.19, -0.15)$) to[out=45, in=135] ($(sadface.center) + (0.19, -0.15)$);

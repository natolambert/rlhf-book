...
% --- nodes (shared vertical centerline at x=0 so they sit centered) ---
\node[agent] (agent) at (0,0)    {Agent\\[4pt]{\LARGE$\pi_\theta(\cdot)$}};
\node[env]   (env)   at (0,-3.7) {Target\\Environment};

% --- action: agent -> environment (right side, down) ---
\draw[loop] (agent.east) -- (3.6,0) -- (3.6,-3.7) -- (env.east);
\node[edgelbl] at (4.05,-1.85) {$a_t$};

% --- state + reward: environment -> agent (left side, up) ---
% labels centered on the vertical segment midpoint (y=-1.85)
\draw[loop] (env.west) -- (-3.6,-3.7) -- (-3.6,0) -- (agent.west);
\node[edgelbl] at (-4.05,-1.45) {$s_t$};
\node[edgelbl] at (-4.05,-2.25) {$r_t$};

...
def draw_ground(ax, y_ground, x_min, x_max):
    ...
    # Ground line
    ax.plot(
        [x_min, x_max], [y_ground, y_ground], color=COLOR_GROUND, linewidth=3, zorder=1
    )
    # Hatching below ground
    for x in np.arange(x_min, x_max, 0.3):
        ax.plot(
            [x, x - 0.15],
            [y_ground, y_ground - 0.15],
            color=COLOR_GROUND,
            linewidth=1,
            zorder=1,
        )
...
def draw_pole(ax, x_cart, y_cart_top, pole_length=1.8, angle_deg=15):
    ...
    pole = Polygon(
        pole_corners,
        facecolor=COLOR_POLE,
        edgecolor=COLOR_POLE_BORDER,
        linewidth=2,
        zorder=4,
    )
    ax.add_patch(pole)
...
def draw_state_labels(ax, x_cart, y_ground, y_cart_top, pole_end_x, pole_end_y):
    ...
    ax.annotate(
        ...
        arrowprops=dict(
            arrowstyle="->", color=COLOR_STATE, lw=2, connectionstyle="arc3,rad=0.5"
        ),
    )
    ax.text(
        pole_end_x + 0.387,
...
def draw_action_arrows(ax, x_cart, y_cart):
    ...
    ax.annotate(
        "",
        xy=(x_cart - 1.3, arrow_y),
        xytext=(x_cart - 0.6, arrow_y),
        arrowprops=dict(arrowstyle="-|>", color=COLOR_ARROW, lw=3, mutation_scale=20),
    )
    ax.text(
        x_cart - 0.95,
        arrow_y - 0.055,
...
def render_diagram(output_path: Path, fmt: str = "png", dpi: int = 300):
    ...
    pole_end_x, pole_end_y, angle_rad = draw_pole(
        ax, x_cart, y_cart_top, pole_length=1.6, angle_deg=pole_angle
    )

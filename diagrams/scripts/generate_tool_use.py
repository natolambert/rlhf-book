...
def render_tool_use_diagram(output_path: Path, fmt: str = "png", dpi: int = 200):
    ...
    # === Title ===
    ax.text(
        x_offset + len(response_tokens) * box_w / 2,
        4.1,
    ...
    # Masked label
    ax.text(
        x_offset + len(prompt_tokens) * box_w + 0.2,
        y_prompt + box_h / 2,
        "(masked)",
        ha="left",
        va="center",
    ...
    ax.annotate(
        "",
        xy=(tool_mid + 0.3, exec_y + 0.25),
        xytext=(tool_mid + 0.3, y_response - 0.1),
        arrowprops=dict(arrowstyle="->", color="#E65100", lw=2),
        zorder=1,
    )
    ax.annotate(
        "",
        xy=(output_mid, y_response - 0.1),
        xytext=(output_mid, exec_y + 0.25),
...
def main():
    ...
    render_tool_use_diagram(
        args.output_dir / f"tool_use_generation.{args.format}",
        fmt=args.format,
        dpi=args.dpi,
    )
    if args.format == "png":

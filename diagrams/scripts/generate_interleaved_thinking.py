...
def render_interleaved_thinking(output_path: Path, fmt: str = "png", dpi: int = 200):
    ...
    fig, (ax1, ax2) = plt.subplots(
        2, 1, figsize=(16, 6), gridspec_kw={"height_ratios": [1, 1], "hspace": 0.35}
    )
    ...
    # =========================================================
    ax1.set_title(
        "Single Turn: The Think / Tool Loop",
        fontsize=14,
        fontweight="bold",
        pad=12,
        loc="left",
    ...
    ax1.plot(
        [bracket_l, bracket_l],
        [bracket_top, bracket_top + tick_h],
        color=loop_color,
        lw=2,
    ...
    ax1.plot(
        [bracket_l, bracket_r],
        [bracket_top + tick_h, bracket_top + tick_h],
        color=loop_color,
        lw=2,
        solid_capstyle="round",
    )
    ax1.plot(
        [bracket_r, bracket_r],
        [bracket_top + tick_h, bracket_top],
        color=loop_color,
        lw=2,
        solid_capstyle="round",
    )
    ...
    ax1.annotate(
        ...
        arrowprops=dict(arrowstyle="-|>", color=loop_color, lw=2, mutation_scale=12),
    )

    mid_loop = (bracket_l + bracket_r) / 2
    ax1.text(
        mid_loop,
        bracket_top + tick_h + 0.08,
        "repeats until ready to respond",
        ha="center",
        va="bottom",
        fontsize=10,
        color=loop_color,
    ...
    # =========================================================
    ax2.set_title(
        "(Optional) Multi-Turn Context Management",
        fontsize=14,
        fontweight="bold",
        pad=12,
        loc="left",
    ...
    for label, w in dropped_blocks:
        draw_block(ax2, x, y0, w, BLOCK_H, label, "dropped", fontsize=10, dashed=True)
        x += w + 0.15
    drop_end = x - 0.15
    ...
    ax2.plot(
        ...
        [drop_bracket_y, drop_bracket_y + drop_tick],
        color=drop_color,
        lw=2,
        solid_capstyle="round",
    )
    ax2.plot(
        [drop_start, drop_end],
        [drop_bracket_y + drop_tick, drop_bracket_y + drop_tick],
        color=drop_color,
        lw=2,
        solid_capstyle="round",
    )
    ax2.plot(
        [drop_end, drop_end],
        [drop_bracket_y + drop_tick, drop_bracket_y],
        color=drop_color,
        lw=2,
        solid_capstyle="round",
    )
    ...
    for style, label in reversed(legend_items):
        ...
        ax1.add_patch(
            FancyBboxPatch(
                (lx, legend_ax_y - swatch_size[1] / 2),
                *swatch_size,
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate interleaved thinking diagram"
    )
    parser.add_argument(
        "--output-dir",
        type=Path,

...
IO_DIAGRAMS = [
    IODiagram(
        ...
        subtitle="Inference: Score a single completion",
        inputs=["prompt x", "completion y"],
        input_label="(x, y)",
        model_name="Reward Model",
        model_components=["LM Trunk", "Linear Head → scalar"],
        output_label="Scalar Score",
        output_example="r(x,y) = 0.73",
        output_type="Single scalar at EOS",
        notes=[
            "Higher score = higher quality",
            "Used to rank completions",
    ...
    IODiagram(
        ...
        subtitle="Inference: Per-token correctness probabilities",
        inputs=["prompt x", "completion y"],
        input_label="(x, y)",
        model_name="ORM",
        model_components=["LM Trunk", "Per-token classifier"],
        processing="Aggregate",
        processing_detail="mean / last / weighted",
        output_label="Correctness Score",
        output_example="p(correct) = 0.91",
        output_type="Aggregated from per-token probs",
        notes=[
            "Each token gets p(correct|t)",
    ...
    IODiagram(
        ...
        subtitle="Inference: Score each reasoning step",
        inputs=["prompt x", "CoT steps s₁...sₖ"],
        input_label="(x, steps)",
        model_name="PRM",
        model_components=["LM Trunk", "3-class head at boundaries"],
        processing="Aggregate",
        processing_detail="min / product / weighted",
        output_label="Process Score",
        output_example="score = 0.85",
        output_type="Aggregated step scores",
        notes=[
            "Score at each step boundary",
            "Classes: correct/neutral/incorrect",
            "Guide search, prune bad paths",
        ],
    ),
    IODiagram(
        name="gen_rm_io",
        title="Generative RM (LLM-as-Judge)",
        subtitle="Inference: Generate natural language verdict",
        inputs=["prompt x", "completion(s) y", "rubric (optional)"],
        input_label="Judge prompt",
        model_name="Full LLM",
        model_components=["GPT-4 / Claude / Llama", "Generate verdict"],
        processing="Parse",
        processing_detail="regex / structured output",
        output_label="Preference / Score",
        output_example="A ≻ B  or  score: 4/5",
        output_type="Extracted from generated text",
        notes=[

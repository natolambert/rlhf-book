...
def draw_label_circle(ax, x, y, label, color="#4CAF50"):
    ...
    circle = plt.Circle(
        (x, y), 0.15, facecolor=color, edgecolor="#2E7D32", linewidth=1.5
    )
    ax.add_patch(circle)
    ax.text(
        x,
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate multi-lane token strip diagrams"
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
    ...
    render_value_diagram(
        args.output_dir / f"value_fn_training.{args.format}", fmt=args.format
    )

...
A viewer asks whether post-training should be sequential rather than use a strong mixture.

**Industry post-training has a surprising number of stages and sequencing choices.** It is rarely a simple linear pipeline with one stage per capability; recipes often fork and merge, with some sequential stages too.
...
A [fun paper](https://arxiv.org/abs/2512.13607)! It was integrated into Nemotron to some extent, but was not accepted as state of the art.

<!-- footnote-right: Source: [@nvidia2025nemotroncascade] -->

---

## Cascade RL 2: A sequential recipe + on-policy distillation
...
Mostly, bigger models tend to need less data (e.g. [Tülu 3 405B](https://allenai.org/blog/tulu-3-405B)). They're more sample efficient at RL (in terms of steps, not total GPU hours) and need less SFT data (or much of the SFT data doesn't help them). I'd read this as bigger models need fewer tokens/capability, which fits scaling laws.
...
Yes — the two updates share their core term, and on curated targets SFT is the "all-positive" special case (the same can be shown for DPO). That shared algebra is what drives iw-SFT [@qin2025supervised], DFT [@wu2025generalization], and the unified-view papers [@lv2025unified].

A huge difference: **negative gradients**. RL pushes below-baseline samples *down* — it learns from its own mistakes, which SFT structurally cannot do. Another is the power of on-policy learning to regularize and nudge a model.
...
Build evaluations to see where you're at, but assume any meaningful training effort is at least millions or tens of millions of dollars.

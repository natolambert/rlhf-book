...
## The similarities are real — the learning signal isn't

Yes — the two updates share their core term, and on curated targets SFT is the "all-positive" special case. That shared algebra is what drives iw-SFT [@qin2025supervised], DFT [@wu2025generalization], and the unified-view papers [@lv2025unified].

<!-- step -->

The huge difference: **negative gradients**. RL pushes below-baseline samples *down* — it learns from its own mistakes, which SFT structurally cannot do. Add on-policy resampling every batch and the gap compounds over a long run.

For the math — and why RL generalizes where SFT forgets — see [Lecture 10](https://www.youtube.com/watch?v=IwpYxANrpUs&list=PLL1tdVxB1CpVpEtMHxwuR4uI4Lxjw00_y&index=15).

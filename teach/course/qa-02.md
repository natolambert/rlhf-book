...
## Notation gotchas in policy-gradient equations

Small notation choices can change what an equation appears to mean:

- **Trajectory sampling**: $\tau \sim \pi_\theta$ is common shorthand, but the trajectory distribution is really $p_\theta(\tau)$, induced by the initial-state distribution, policy, and environment dynamics.
- **Reward indexing**: pick one convention. Here we use zero-indexed per-step rewards, e.g. $G_t = \sum_{k=0}^{\infty}\gamma^k r_{t+k}$; reserve $R(\tau)$ or $R_i$ for full trajectory / completion rewards.
- **Score-function terms**: write the scalar outside the derivative, e.g. $\Psi_t \nabla_\theta \log \pi_\theta(a_t \mid s_t)$, so it is clear $\Psi_t$ is a weight, not part of $\nabla_\theta$.

Most papers use acceptable shorthand somewhere; the goal is to know which object is distributional, which is sampled, and which term receives gradients.

<!-- footnote-right: Source: Chapter 6 notation errata discussion ([issue #464](https://github.com/natolambert/rlhf-book/issues/464)) -->
...
## The clipped objective, region by region

![The PPO objective $J(\theta)$ vs. the policy ratio $\rho(\theta)$ for positive and negative advantage. Each panel annotates the three ratio regions with their unclipped term, clipped term, resulting objective, and gradient — the active branch takes a standard policy-gradient step; the clipped branch flattens to zero gradient.](assets/ppo-clip-viz.png)

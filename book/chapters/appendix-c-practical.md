...
2. Run multiple seeds on the best few settings. Random seed can have meaningful impacts on the final model, and it's worth spending compute on.
...
The best way to understand this is to evaluate many models on a largely static evaluation suite. Over time, you develop an intuition for which tests are hard to move with post-training interventions (often knowledge-heavy evaluations such as MMLU).

...
Following Kim & Rush's notation, let $s$ be the source sentence or prompt, $t = (t_1,\ldots,t_J)$ be the target sequence from the teacher model, $\mathcal{V}$ be the output vocabulary (possible tokens in the tokenizer), $q$ be the teacher distribution over next-tokens, and $p$ be the student distribution.
Note that their paper calls this word-level distillation, but for modern language models this is best read as per-token distribution matching over the tokenizer vocabulary, as it came before modern sub-word tokenizers:

$$
\mathcal{L}_{\mathrm{WORD-KD}}
= -\sum_{j=1}^{J}\sum_{k=1}^{|\mathcal{V}|}
q(t_j = k \mid s, t_{<j})\log p(t_j = k \mid s, t_{<j}).
$$ {#eq:word_kd}
...
Sequence-level distillation instead treats $\mathcal{T}$ as the space of possible target sequences and matches the student to the teacher distribution over full sequences.
Because the sum over all complete sequences $t \in \mathcal{T}$ is intractable, requiring summing over an exponential number of potential sequences, Kim & Rush approximate the teacher distribution over sequences with a point mass on a single high-probability teacher output $\hat{y}$.
Here $\hat{y}$ is a sequence produced by beam search with the teacher model, so $\hat{y} = \mathrm{BeamSearch}_q(s) \approx \arg\max_{t \in \mathcal{T}} q(t \mid s)$:

$$
\begin{aligned}
\mathcal{L}_{\mathrm{SEQ-KD}}(s)
= -\sum_{t \in \mathcal{T}} q(t \mid s)\log p(t \mid s)
\approx -\log p(\hat{y} \mid s) \\
= -\sum_{j=1}^{|\hat{y}|}\log p(\hat{y}_j \mid s, \hat{y}_{<j}).
\end{aligned}
$$ {#eq:sequence_kd}
...
These *offline* KD algorithms had a few limitations hindering adoption.
...
For example, the reverse KL optimization tends to push student models to  over-estimate low-probability areas of the teacher distribution.
Together, these issues issues were an opening for *On-policy* Distillation (OPD).

The core shift to on-policy distillation is the idea that we can tweak the optimization by sampling from the student models, and measure the distance of the distribution of the teacher, rather than originally sampling from the teacher model.
MiniLLM noted the need to shift to a reverse KL optimization (we explain intuitively why this target can be better in Chapter 15) and proposed using KD loss functions within a online policy-gradient RL framework [@gu2024minillm].
Other concurrent work [@agarwal2024policy] showed the promise of on-policy KD and connected the iterate process of generating from the student and grading with a teacher to iterative, imitation learning work from the RL literature (e.g. DAgger [@ross2011reduction]).
...
The direction $D_{\mathrm{KL}}(\pi_\theta \| \pi_T)$ makes this a reverse-KL objective from the student's perspective:
...
Here, we have shifted to the expectation notation, as used extensively in 6 which covers the fundamental RL policy-gradient algorithms, as the optimization is solved by sampling trials and numerically estimating the gradient.
This shift to the sampling framework acts as a natural transition to modern LLM training infrastructure with RL, which is designed to rapidly alternative between generating tokens from the current policy being trained and taking learning updates.

In fact, recent impelmentations of OPD take this integration of KD with RL a step further, where the KD distance is taken directly as a reward signal within the RL optimization.
...
The negative of the KL distance is used because the optimization is designed to minimize the distance between the student and teacher, where a large positive value indicates a big token mismatch, and the advantage must be a positive number that the RL algorithm maximizes.
...
For multiple teachers, let $\pi_{T_k}$ be teacher $k$ and let $w_k$ be its mixture weight, typically with $\sum_k w_k = 1$.
The same reverse-KL objective can be averaged across teacher distributions:

$$
\mathcal{L}_{\mathrm{MOPD}}(\theta)
= \mathbb{E}_{x, y \sim \pi_\theta(\cdot \mid x)}
\sum_t \sum_k w_k D_{\mathrm{KL}}\left(\pi_\theta(\cdot \mid s_t) \;\|\; \pi_{T_k}(\cdot \mid s_t)\right).
$$ {#eq:mopd_objective}
...
- TODO: Connect iterative distillation to the DAgger imitation-learning framework . Hint: the shared problem is covariate shift from training only on expert/offline trajectories, and the shared remedy is collecting supervision on states induced by the current learner.

...
For each teacher, a contribution weight is decided per-prompt in the training batch, in order to create Multi-Teacher On-Policy Distillation (MOPD) [@mimo2025flash].
For multiple teachers, let $\pi_{T_k}$ be teacher $k$ and let $w_k$ be its mixture weight (with $\sum_k w_k = 1$) within the reverse KL loss:

...
- 2026-06-29: [PR #429](https://github.com/natolambert/rlhf-book/pull/429) changed SDPO rollout collection from filter-down to poll-until-full: each step now keeps polling the inference engine over a cycling prompt stream until `prompts_per_step` prompts each yield a correct rollout, so every update sees a fixed effective batch instead of `prompts_per_step − skipped`. This changes comparability — `skipped` now counts polled prompts with no correct rollout (unbounded per step) rather than capping the batch. Config: `prompts_per_step` 16→8, `data.size` 3000→10000.
- 2026-06-28: [PR #429](https://github.com/natolambert/rlhf-book/pull/429) migrated the SDPO `distillation/` module from LiveCodeBench to Reasoning Gym (default `spell_backward` string-reversal task), mirroring the `policy_gradients` GRPO data setup. Removed all code-execution and compiler-feedback machinery: completions are now verified by `dataset.score_answer`, and the self-teacher is conditioned only on a correct sibling demonstration. Prompts whose rollout group contains no fully-correct demonstration are skipped entirely (no update). Scoring collapses to a single `avg_reward` (mean environment score); configs replace `split`/`max_tests` with a Reasoning Gym `data.specs` mixture.
...
- 2026-05-10: [PR #416](https://github.com/natolambert/rlhf-book/pull/416) added `instruction_tuning/` module — single-GPU SFT example for the chapter 4 sanity check (base model rambles vs SFT model answers and stops). Defaults to `allenai/OLMo-2-0425-1B` on `HuggingFaceH4/no_robots`; lifts the chat template from `OLMo-2-0425-1B-SFT` since the base tokenizer ships without one. Prints a fixed prompt pool (incl. capital-of-France) to the console at step 0 and every `sample_every` optimizer steps; loss/LR go to W&B, sample text is console-only. Follow-up: pass `return_dict=False` to `apply_chat_template` (transformers v5 default flipped to `BatchEncoding`, which silently broke label masking), drop the `masking` config knob (always train on the final assistant turn), drop the multi-format `_normalize_messages` adapter (only `messages`-style rows are supported), and render in-loop generation samples in colored panels with special tokens visible.
...
- 2026-05-03: [PR #393](https://github.com/natolambert/rlhf-book/pull/393) fixed rejection-sampling imports and `progress_bar` arguments
- 2026-05-03: [PR #391](https://github.com/natolambert/rlhf-book/pull/391) collapsed the five separate per-completion reward fields on `Experience` (`rewards`, `correctness`, `format`, `penalties`, `binary_reward`) into a single `rewards: TensorDict` keyed by `total / correctness / format / penalty / binary`. `compute_rewards` now returns the TensorDict directly (including `binary`), and `apply_reward_kl` / `compute_advantages` consume it instead of taking the components as separate arguments. Adds `tensordict` as a dependency. Wandb metrics are now logged as `avg_<key>` (`avg_total`, `avg_correctness`, `avg_format`, `avg_penalty`, `avg_binary`) and are derived from the TensorDict keys, so adding a new reward component surfaces a new metric automatically — prior wandb run metric names (`avg_reward`, `avg_correctness_reward`, `avg_format_reward`, `avg_response_penalty`, `avg_binary_reward`) are no longer comparable by name.

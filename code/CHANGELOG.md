...
- 2026-06-12: [PR #TBD](https://github.com/natolambert/rlhf-book/pull/0) made the reward-model exercises runnable on `datasets>=5.0`/`transformers>=5.0` and added study material. Fixed ORM's dataset id (`gsm8k` → `openai/gsm8k`; the bare canonical name now raises `HfUriError`) and made PRM's `demo_scoring` resilient to a `datasets>=5.0` streaming schema-cast error on the PRM800K *test* split (`int64`→`null`) by falling back from `test` to `train`. Added `reward_models/rm_preflight.py` (read-only data/tokenizer preflight that exercises the real `build_*_dataset` paths) and `reward_models/NOTES.md` (per-model input/output/loss/objective breakdown plus base-vs-SFT init notes). Corrected the `base.py` docstring that called the default setup a "frozen backbone" (it is a full fine-tune) and documented ORM's noisy synthetic-negative construction inline.

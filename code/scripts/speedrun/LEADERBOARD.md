...
# Speedrun with target reward 1.35 (goal = 100 rollout-buffer iteration avg)
...
The script writes Date, model, dataset, goal@step, time-to-target, run_id, walltime, final_reward, algorithm, beta, and Notes to the table. For multiple runs, you can list your best or a representative run.
...
- **goal@step**: Target reward and first reached step (e.g. `goal(1.35)@step181`, based on 100 rollout-buffer iteration rolling average)
- **time_to_target**: Walltime at first target reach (based on 100 rollout-buffer iteration rolling average)
...
- **final_reward**: `avg_total` at the last rollout-buffer iteration
- **algorithm**: Algorithm (rloo / ppo / drgrpo / gspo / cispo / reinforce, etc.)
- **beta**: KL penalty coefficient (`cfg.beta`; relevant when comparing `avg_total` across PPO/RLOO/REINFORCE runs)
...
|------|--------|-------|---------|-----------|----------------|--------|----------|--------------|-----------|------|-------|-------|
| 2026-04-09 | shota | Qwen/Qwen3-1.7B | spell_backward | goal(1.4)@step387 | 15 h 20 min 4 sec | 05nfj1m9 | 19 h 1 min 18 sec | 1.4688 | grpo | | [run](https://wandb.ai/shotakaji-independent-researcher/rlhf-book/runs/05nfj1m9) |  |
| 2026-04-10 | shota | Qwen/Qwen3-1.7B | spell_backward | goal(1.4)@step474 | 19 h 4 min 26 sec | uh77y0qo | 20 h 2 min 19 sec | 1.4688 | cispo | | [run](https://wandb.ai/shotakaji-independent-researcher/rlhf-book/runs/uh77y0qo) |  |

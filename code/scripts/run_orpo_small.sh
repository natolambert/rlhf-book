#!/bin/bash
# Launch a quick ORPO sanity run in the background.
...
export WANDB_RUN_NAME="${WANDB_RUN_NAME:-orpo-small-${ts}}"
...
LOG_FILE="/tmp/orpo-small-${ts}.log"
...
if [ "${RUN_IN_BACKGROUND}" = "1" ]; then
  ...
  if ! ps -p "${PID}" >/dev/null 2>&1; then
    echo "ORPO run exited early. Last log lines:"
    tail -n 80 "${LOG_FILE}" || true
    exit 1
  fi

  echo "Started ORPO small run."
  echo "  PID: ${PID}"
  echo "  Log: ${LOG_FILE}"
  ...
  echo "Running ORPO in foreground."
  echo "  Log: ${LOG_FILE}"
  echo "  Run: ${WANDB_RUN_NAME}"
